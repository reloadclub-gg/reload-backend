...
class Team(BaseModel):
    ...
    def players_count(self) -> int:
        ...
        """
        return sum(
            [len(Lobby.get(lobby_id).players_ids) for lobby_id in self.lobbies_ids]
        )
    ...
    def overall(self) -> int:
        ...
        if len(self.lobbies_ids) > 0:
            return ceil(
                mean([Lobby.get(lobby_id).overall for lobby_id in self.lobbies_ids])
            )
    ...
    def lobbies(self) -> list[Lobby]:
        ...
        Return lobbies.
        """
        return [Lobby.get(lobby_id) for lobby_id in self.lobbies_ids]
    ...
    def create(lobbies_ids: list) -> Team:
        ...
        """
        lobbies = [Lobby.get(lobby_id) for lobby_id in lobbies_ids]
        max_players = [lobby.max_seats for lobby in lobbies]
        players_count = sum([len(lobby.players_ids) for lobby in lobbies])
        ...
        if not all([lobby.is_queued for lobby in lobbies]):
        ...
        for lobby in team.lobbies:
            cache.set(f'{lobby.cache_key}:team_id', team_id)
    ...
    def delete(self):
        ...
        for lobby in self.lobbies:
            ...
            cache.delete(f'{lobby.cache_key}:team_id')
    ...
    def handle_non_queued_lobbies(self):
        non_queued = [lobby for lobby in self.lobbies if not lobby.is_queued]
