...
def log_teaming_info():
    ...
    separator_line = "+" + "-" * 40
    logging.warning(title_separator_line)
    logging.warning("| lobbies.tasks.handle_teaming")
    logging.warning(title_separator_line)
    logging.warning("| unready teams:")
    logging.warning(separator_line)
...
def handle_teaming():
    ...
    for lobby in queued_lobbies:
        ...
        for player_id in lobby.players_ids:
            if PreMatch.get_by_player_id(player_id):
                skip_lobby = True
        ...
        if not lobby_team:
            for team in unready_teams:
                if (
                    lobby.max_players
                    and team.players_count + lobby.players_count <= lobby.max_players
                ):
